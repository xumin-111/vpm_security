        ...
        checkboxTree: {
            ...
            init: function (options) {
                ...
                $.get(options.url, function (data) {
                    ...
                    var node = tree.getNodesByParam("id", treeId, null)[0];
                    $.checkboxTree.selectByIdName(treeId, node);
                });
            },
            // 搜索节点
            searchNode: function () {
                // 取得输入的关键字的值
                var value = $.common.trim($("#keyword").val());
                if ($.checkboxTree._lastValue == value) {
                    return;
                }
                // 保存最后一次搜索名称
                $.checkboxTree._lastValue = value;
                var nodes = $._tree.getNodes();
                // 如果要查空字串，就退出不查了。
                if (value == "") {
                    $.checkboxTree.showAllNode(nodes);
                    return;
                }
                $.checkboxTree.hideAllNode(nodes);
                // 根据搜索值模糊匹配
                $.checkboxTree.updateNodes($._tree.getNodesByParamFuzzy("name", value));
            },
            // 根据Id和Name选中指定节点
            selectByIdName: function (treeId, node) {
                if ($.common.isNotEmpty(treeId) && treeId == node.id) {
            ...
            showAllNode: function (nodes) {
                ...
                for (var i = nodes.length - 1; i >= 0; i--) {
                    ...
                    $._tree.showNode(nodes[i]);
                    $.checkboxTree.showAllNode(nodes[i].children);
                }
            },
            // 隐藏所有节点
            hideAllNode: function (nodes) {
            ...
            showChildren: function (treeNode) {
                if (treeNode.isParent) {
                    for (var idx in treeNode.children) {
                        var node = treeNode.children[idx];
                        $._tree.showNode(node);
                        $.checkboxTree.showChildren(node);
                    }
                }
            },
            // 更新节点状态
            updateNodes: function (nodeList) {
                ...
                for (var i = 0, l = nodeList.length; i < l; i++) {
                    var treeNode = nodeList[i];
                    $.checkboxTree.showChildren(treeNode);
                    $.checkboxTree.showParent(treeNode)
                }
            },
            // 获取当前被勾选集合
            getCheckedNodes: function (column) {
